const dialpadArr = [1, 2, 3, 4, 5, 6, 7, 8, 9, "*", 0, "#"];
...
(function ($) {
  ...
  let _VSansayClientSpeakerState = 50;
  ...
  function toggleSansaySpeaker(vol){
    _oSansayClient.setVolumeState(vol)
  }

  function _fSansayUiLogoInit() {
    $(".webphone-logo").attr("src", "../images/ringplan-sm-blue.png");
  }

  function _fSansayUiKeypadInit() {
    const keypadData = dialpadArr
      .map((item) => {
        const id = getTitle(item);
      ...
      <div
        ...
        class="bg-white ${id === 1 ? 'pb-2' : ''} hover:bg-gray-500 flex-col
              ...
              text-[#42353B] cursor-pointer
                ...
                select-none
                items-center h-18.5 w-18.5 rounded-full">
                ${item}
                <small class="text-2.5 leading-2.5 font-light">${subText}</small>
      </div>
        `;
      })
      .join("");

    $("#dialpad-container").prepend(keypadData);

    // fcn keys
    $("#webphone-redial-btn").click(function () {
      _fSansayUiRedialKey();
    });
    $("#webphone-backspace-btn").click(function () {
      _fSansayUiBackspace();
    });
    ...
    // digit keys
    $("#webphone-1-btn").click(function () {
      _fSansayUiRegularKey("1");
    });
    $("#webphone-2-btn").click(function () {
      _fSansayUiRegularKey("2");
    });
    $("#webphone-3-btn").click(function () {
      _fSansayUiRegularKey("3");
    });
    $("#webphone-4-btn").click(function () {
      _fSansayUiRegularKey("4");
    });
    $("#webphone-5-btn").click(function () {
      _fSansayUiRegularKey("5");
    });
    $("#webphone-6-btn").click(function () {
      _fSansayUiRegularKey("6");
    });
    $("#webphone-7-btn").click(function () {
      _fSansayUiRegularKey("7");
    });
    $("#webphone-8-btn").click(function () {
      _fSansayUiRegularKey("8");
    });
    $("#webphone-9-btn").click(function () {
      _fSansayUiRegularKey("9");
    });
    $("#webphone-star-btn").click(function () {
      _fSansayUiRegularKey("*");
    });
    $("#webphone-0-btn").click(function () {
      _fSansayUiRegularKey("0");
    });
    $("#webphone-pound-btn").click(function () {
      _fSansayUiRegularKey("#");
    });
    ...
    //   _VSansayClientSpeakerState ^= 86;
    //   toggleSansaySpeaker(_VSansayClientSpeakerState)
    // })
    ...
    $("#webphone-video-end-btn").click(function () {
      _fSansayUiCallKey(1);
    });
  }
  ...
  function _fSansayUiBackspace() {
    ...
    else $(".webphone-digits").html("");

    if ($(".webphone-digits").text().length === 0) {
      document.querySelector("#webphone-backspace-btn").classList.add("hidden");
    }
  }
  ...
  function _fSansayUiMuteKey() {
    _vSansayUiMute ^= 1;
    $("#webphone-mute-btn > img").attr('src', _vSansayUiMute ? '/images/mute.svg' : '/images/unmute.svg')
  }
  ...
  function _fSansayUiCallKey(video_call) {
    ...
    if (_vSansayUiCallStateActive) {
      ...
      if (called_num.length <= 0) return;

      _vSansayUiVideoCall =
        typeof video_call != "undefined" && video_call != null ? 1 : 0;
      if (_vSansayUiVideoCall) {
        _fSansayUiVideoPanel();
        _oSansayClient.setMediaElements(
          "local-side-video",
          "remote-side-video"
        );
      }
      ...
      } else _oSansayClient.endRTCSession();
    }
  }

  function _fSansayUiCallActiveUpdate(updaters) {
    if (Array.isArray(updaters) == false) updaters = [updaters];

    for (var i = 0; i < updaters.length; i++) {
      if (typeof updaters[i] === "function") updaters[i]();
    }
    _vSansayUiCallElapsedTimer = setTimeout(
      _fSansayUiCallActiveUpdate,
      2000,
      updaters
    );
  }
  ...
  function _fSansayUiCallStateHandling(active, remote_num) {
    ...
    function __elapsed_update() {
      var now = new Date();
      var elapsed_time =
        parseInt(now.getTime() / 1000) - _vSansayUiCallStartTime;

      var time_str = __elapsed_time_str(elapsed_time);
      $(".webphone-elapse-time").each(function () {
        $(this).html(time_str);
      });
    }
    function __video_aspect_ratio_update() {
      ...
      if (_vSansayUiRemoteAspectRatio != rar) {
        ...
        if (rar < 1) {
          $("#remote-side-video").css(
            "width",
            $(".webphone-video-frame").css("width")
          );
          ...
          $("#remote-side-video").css(
            "height",
            $(".webphone-video-frame").css("height")
          );
          $("#remote-side-video").css("width", "auto");
          var rh_offset = Math.round(
            ($(".webphone-video-main").width() -
              $(".webphone-video-frame").width()) /
              4
          );
      ...
      if (_vSansayUiLocalAspectRatio != lar) {
        ...
        if (lvw / lvh < 1) {
          $("#local-side-video").css(
            "width",
            $(".webphone-pip-frame").css("width")
          );
          ...
          $("#local-side-video").css(
            "height",
            $(".webphone-pip-frame").css("height")
          );
          $("#local-side-video").css("width", "auto");
          var lh_offset = Math.round(
            ($(".webphone-video-pip").width() -
              $(".webphone-pip-frame").width()) /
              4
          );
    ...
    if (active) {
      ...
      let netWorkImageSrc = navigator.connection.downlink < 5 ? '/images/wifi-slow.svg' : '/images/wifi.svg'
      $("#last-call").html("");
  ...
  function _fSansayUiVideoPanel() {
    ...
    var rh_offset = Math.round(
      ($(".webphone-video-main").width() - $(".webphone-video-frame").width()) /
        4
    );
    $(".webphone-video-main").css("left", "-" + rh_offset + "px");
    var lh_offset = Math.round(
      ($(".webphone-video-pip").width() - $(".webphone-pip-frame").width()) / 4
    );
    $(".webphone-video-pip").css("left", "-" + lh_offset + "px");
  ...
  $.fn.webphone = function (server, options) {
    ...
    // load pkg js files
    $.getScript("lib/detect-mobile.js", function () {
    ...
    */
    ...
    this.load("webphone-plugin.html", function () {
      ...
      $('[data-toggle="tooltip"]').hover(function () {
        if (options == undefined || options.position != "left")
          $(".tooltip").css(
            "left",
            parseInt($(".tooltip").css("left")) - 80 + "px"
          );
        $(".tooltip").css(
          "top",
          parseInt($(".tooltip").css("top")) + 20 + "px"
        );
      });
    ...
    if (options != undefined) {
      ...
      if (options.theme != undefined) {
        if (options.theme == "light") theme = "light";
      }
    }
    $("<link/>", {
      rel: "stylesheet",
      type: "text/css",
      href: "css/webphone-light.css",
    }).appendTo("head");
    $("<link/>", {
      rel: "stylesheet",
      type: "text/css",
