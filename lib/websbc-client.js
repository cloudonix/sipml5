  ...
  this.peerConnection.onaddstream = function (event) {
    ...
    self.attachMediaStream(remote_media, event.stream);
      ...
      document.getElementById("volume").addEventListener('change', (e) => {
        ...
        let remoteAudio = document.getElementById(self.remote_media_dom_id)
    ...
    switch(event) {
      case "E_USER_MAKE_CALL":
        self.mediaEngine.open(
          ...
          self.userHangup.bind(self), // self.userHangup, need to fix this
          self.processSDP);
  ...
  switch(msg.messageType) {
    ...
    case "ERROR":
      if (msg.errorType == "NOMATCH") {
        ...
        document.querySelector("#dialpad-content").classList.add("hidden")
        document.querySelector("#login-content").classList.remove('hidden')
        let error = document.createElement("span")
        error.style.color = "red"
        error.id = "error-field"
        error.innerText = "Invalid credentials"
        const target = document.querySelector('#btn-wrapper')
...
SansayWebSBCClient.prototype.setVolumeState = function(vol) {
  if (this.rtc == 0)
    return;
  console.log(this.media.remote_media_dom_id, 'media remote dom id')
