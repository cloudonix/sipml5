...
BUILD_VERSION = "2.2.1";
BUILD_DATE    = "Tue Feb 20 20:17:28 PST 2018";
BUILD_BY      = "martin";
...
function parseCallStack(emsg) {
  ...
  var fcn_name = caller_info[caller_info.length - 2];
  var line_info = caller_info[caller_info.length -  1].split(/[\/\(\)]+/);
...
//  if (!__quiet && !this.quiet)
    ...
    let info = parseCallStack(e.stack.toString());
    console.error(info[0] + " at " + info[1] + "\n  ERROR: [" + this.module + "] " + msg);
};



/********************************************************************
 websbc media layer: without FSM. should only be used as a backward reference
*********************************************************************/
...
function WebRTC() {
  ...
  if (navigator.webkitGetUserMedia) {   // chrome
    ...
    if (navigator.userAgent.match(/Edge\/(\d+).(\d+)$/)) { // edge
      self.browser = "EDGE";
    ...
    else if (navigator.userAgent.match(/AppleWebKit\/(\d+)\./)) { // safari
      self.browser = "SAFARI";
      self.browser_version = parseInt(navigator.userAgent.match(/(applewebkit)\/(\d+)\./i)[2]);
    }
    else {
      alert("current browser does not support WebRTC");
    }
  }
  else {
    alert("current browser does not support WebRTC");
  ...
  webrtc_logger.debug("*** Browser: " + self.browser + " version: " + self.browser_version + " ***");
}
...
WebRTC.prototype.placeCall = function() {
  if (this.peerConnection == null) {
    alert("WebRTC not opened");
    return false;
  }
  if (this.localStream == null) {
    alert("WebRTC local streaming is not ready");
    return false;
  }
...
ChanWebSocket.prototype.open = function(sig_server, sig_controller_path, user_id, conn_change_handler) {
  ...
  if (server_config.length < 2 || isNaN(parseInt(server_config[1])))
    this.sock = new WebSocket("wss://" + server_config[0] + ":80/" + sig_controller_path, "sansay-roap", "test");  // origin=test
  else
...
function edgeRTCPeerConnection(config) {
  edgeVersion = parseInt(navigator.userAgent.match(/edge\/(\d+).(\d+)$/i)[2]);
  ...
  this.dispatchEvent       = _eventTarget['dispatchEvent'].bind(_eventTarget);
  ...
  if (config.iceCandidatePoolSize) {
    for (var i = config.iceCandidatePoolSize; i > 0; i--) {
      this._iceGatherers = new window.RTCIceGatherer({iceServers: config.iceServers,
...
edgeRTCPeerConnection.prototype._createTransceiver = function(kind) {
...
edgeRTCPeerConnection.prototype._createIceGatherer = function(sdpMLineIndex, usingBundle) {
  var pc = this;
  ...
  var iceGatherer = new window.RTCIceGatherer({iceServers: this._config.iceServers,
...
edgeRTCPeerConnection.prototype.createOffer = function() {
  ...
  /* NOTE: we do our own callback way instead of promise
